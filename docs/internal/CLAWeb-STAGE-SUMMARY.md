...
### 2.2 公网网页入口
- 现网已存在 `/claweb/` 新网页入口，与旧 webchat 并行。
- 新入口已完成真实公网访问与用户侧消息往返验证。
...
### 6.1 CLAWeb 的定位
CLAWeb 应被视为 **OpenClaw-compatible frontend / interface layer**，而不是一套独立协议、独立媒体系统或独立生成框架。

它的职责是：
- 承接 OpenClaw 已有消息与媒体输出；
- 提供 Web 侧 frontdoor / history / ws / 前端展示能力；
- 在不分叉协议的前提下增强网页体验。

### 6.2 兼容方向
后续原则应固定为：
- **不是让 OpenClaw 适配 CLAWeb；**
- **而是让 CLAWeb 尽量兼容 OpenClaw。**
...
> CLAWeb 当前已完成文本闭环、公网测试入口、图片链路收口、富文本安全子集、reply preview 摘要与历史回填；UI 已够用但仍待长期打磨。下一步优先做工程收口，再做 markdown 能力扩展。
